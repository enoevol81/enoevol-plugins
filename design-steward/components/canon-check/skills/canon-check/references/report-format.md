...
directory if needed; recovery follows the installed cleanup plugin contract). Use this section order; omit a category entirely
...
## Decision register
| Finding ID | Scope (app/component/theme/breakpoint) | Decision status | Authority / exception | Source baseline |
| --- | --- | --- | --- | --- |
| radius-button | app/button/all/all | unresolved | Doc says 8px; source uses 6px | sources.json |

Decision status is observed, proposed, approved, superseded or unresolved.
Repetition never establishes approval. Compare conflicts within the same semantic
role and scope. Link every category finding below to a register ID.
