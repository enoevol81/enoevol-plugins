...
Read `../../references/decision-contract.md` for scoped decision status, freshness
checks, incremental updates and current recovery behavior. These rules apply to
both modes. Frequency establishes an observation, never approval.
...
6. **Save the source snapshot** using `../../references/decision-contract.md`, then **write the report** to `_canon-check/<date>/canon-report.md` using the
...
1. Check for an existing `_canon-check/<date>/canon-report.md`. If one exists
   and covers the relevant category, check its source snapshot before reuse.
   Refresh stale findings and related consumers; do not trust the date alone.
