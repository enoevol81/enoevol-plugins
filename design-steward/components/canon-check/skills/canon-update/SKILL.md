...
description: >-
  ...
  silent batch rewrite — use existing decisions and resolve unanswered choices.
---
...
translate that intent into precise document edits, apply existing authorization and resolve unanswered choices,
...
direction — so it needs both sides. Check source freshness with `../../references/decision-contract.md` before reusing a report.
...
Do not guess where the product is headed. First reuse explicit decisions already
provided. Ask only unanswered questions, grounded in
...
If direction is still unclear, ask the one open-ended question the report can't answer: *"Beyond
...
Carry forward explicit user decisions and existing authorization. Present a
single grouped diff when unresolved choices require approval. Do not demand a
second confirmation for already-authorized edits. If scope changes, ask narrowly
about the new decision. Record approvals and unresolved choices in the update log.
Follow `../../references/decision-contract.md` for status, scope and recovery.
...
- **Deletions go through a safety net, not straight to `rm`.** For each
  ...
  (read its current local-archive and verified-backup contract) — that
...
- **Not a batch rewrite.** Document changes must remain within the user-authorized scope. No "I went ahead and updated everything."
