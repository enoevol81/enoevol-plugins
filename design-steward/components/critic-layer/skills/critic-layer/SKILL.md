...
Read `../../references/handoff.md` for recovery, stable issue IDs, source mapping
and verification. Follow that contract throughout this workflow.
...
When the user says done, read `JSON.stringify(window.__CRITIC__.export())`, take one annotated screenshot, and (for multiple breakpoints) resize and repeat. Save the merged export as `annotation_manifest.json`. **Collect before any navigation or reload** — reinject after reload to recover tab-local notes; export to disk before leaving the tab — and quietly re-collect every ~10 notes on long reviews. Details in `references/capture.md`.
...
- **Navigation wipes notes**: a full page load removes the overlay UI; reinjection restores tab-local captures when storage is available. Collect the
