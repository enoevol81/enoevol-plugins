...
navigation, reload, or new URL** — a full page load removes the overlay UI; reinjection can restore tab-local
...
- **`anchor`** — identity for re-finding the element on re-render: prefer
  `selector`; `id` is validated against the stored tag/text; `text`+`classes` disambiguate repeated components.
...
Pins/edits reposition on scroll/resize by resolving `anchor` each frame (id/selector/class candidates validated against the snapshot; ambiguous matches remain unresolved). Notes survive sticky headers and lazy loads; heavy route re-renders can orphan a pin (`anchorLive` → false). A dead-anchored pin from a *different* SPA route is hidden on the current one (its data stays in the export). Drawings reposition by page coordinates, independent of the DOM. If a re-render tears the overlay's own DOM out, re-injecting rebuilds the HUD and recovers notes, drawings and edit records. See `../../../references/handoff.md` for tab storage, disk exports and import. Imported edits are records only.
