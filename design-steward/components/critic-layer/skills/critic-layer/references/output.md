...
- One `export()` = one capture; collect one per (url, viewport) into `captures`, carrying the export's fields through (`viewportSize`, `capturedAt`, …). Preserve sessionId and issueId unchanged across merges. Deduplicate repeated snapshots by issueId; do not rename IDs by viewport. For legacy imports assign and persist a session ID once.
- **Notes** are intent. **Drawings** localize/emphasize (their `bbox` + the screenshot say *where*; fold a labeled drawing in like a note, an unlabeled one as visual support for a nearby note/edit). **Edits** are the highest-fidelity input: each carries an exact `changes` diff the designer authored live — translate those into tasks preserving the concrete values **verbatim as evidence**, then checking token and responsive scope before proposing source implementation. An edit with a `text` change is a copy directive; a `style` change is a precise value directive (`font-size 34px→40px`); an `html` change is a structural directive.
...
Read `../../../references/handoff.md`. Each issue keeps its original issueId,
authoredBy, sourceCapture, URL/viewport, anchorStatus, scope, decisionStatus,
acceptance criteria, sourceCandidates, implementationStatus and evidence.
An edit preview is not approval of a global token change. The brief and prompt
must preserve unresolved mappings and responsive requirements.
